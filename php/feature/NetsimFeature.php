...
class FakeRestNetsimFeature extends FakeRestBaseFeature
{
    ...
    private function _simulate(FakeRestContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(FakeRestContext $ctx, array $applied): void
    {
        $client = $this->client;
