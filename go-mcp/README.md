...
[MCP](https://modelcontextprotocol.io) server exposing the FakeRest SDK as
two agent tools — `fake-rest_list` and `fake-rest_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fake-rest-mcp)
make build

# 2. Provide credentials via the environment
export FAKE_REST_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user fake-rest \
  -- /absolute/path/to/fake-rest-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./fake-rest-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// fake-rest_list: first page of records
{ "entity": "category" }
{ "entity": "category", "query": { } }

// fake-rest_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fake-rest-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user fake-rest \
     -- "$PWD"/dist/*/fake-rest-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `fake-rest_list` and `fake-rest_load` tools now appear
   in new sessions. Ask the agent to *"list category using fake-rest"*
   and it calls `fake-rest_list` with `{"entity":"category"}`.
...
```sh
export FAKE_REST_APIKEY=sk_live_xxx            # API key
export FAKE_REST_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `fake-rest_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `fake-rest_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FAKE_REST_APIKEY` | API key sent with every request. |
| `FAKE_REST_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

category | comment | post | product | todo | user

### Smoke test via HTTP (raw JSON-RPC)

```sh
./fake-rest-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"fake-rest_load","arguments":{"entity":"post","query":{"id":1}}}}'
```
