# FakeRest TypeScript SDK Reference

Complete API reference for the FakeRest TypeScript SDK.
...
```ts
new FakeRestSDK(options?: object)
```
...
#### `FakeRestSDK.test(testopts?, sdkopts?)`
...
**Returns:** `FakeRestSDK` instance in test mode.
...
Return the parent `FakeRestSDK` instance.
