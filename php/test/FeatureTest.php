...
class FtCtrl extends FakeRestControl
{
...
class FtClient
{
    ...
    private FakeRestUtility $utility;

    public function __construct(FakeRestUtility $utility, array $options)
    {
    ...
    public function get_utility(): FakeRestUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FakeRestContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FakeRestContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FakeRestContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FakeRestConfig::make_feature($name);
    ...
    private static function buildUrl(FakeRestSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FakeRestError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FakeRestResponse($fetched);
        }
    ...
    private function fail(FakeRestContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FakeRestContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
