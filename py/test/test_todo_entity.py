...
class TestTodoEntity:
    ...
    def test_should_stream(self):
        ...
        base = FakeRestSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FAKE_REST_TEST_TODO_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        todo_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.todo")))
        todo_ref01_data = None
        if len(todo_ref01_data_raw) > 0:
            todo_ref01_data = helpers.to_map(todo_ref01_data_raw[0][1])
...
def _todo_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("FAKE_REST_TEST_TODO_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("FAKE_REST_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("FAKE_REST_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FAKE_REST_TEST_EXPLAIN") == "TRUE",
        "live": _live,
