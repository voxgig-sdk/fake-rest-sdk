...
(0, node_test_1.describe)('CommentDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-comment', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-comment', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["post01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.post_id = setup.idmap['post01'];
        }
        else {
...
function liveScenariosActive() { return false && process.env.FAKE_REST_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    const calls = [];
    const env = (0, utility_1.envOverride)({
        'FAKE_REST_TEST_COMMENT_ENTID': {},
    ...
    if (live) {
        ...
        const client = new __1.FakeRestSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['FAKE_REST_TEST_COMMENT_ENTID'];
    ...
    const client = new __1.FakeRestSDK({
        base: 'http://localhost:8080',
