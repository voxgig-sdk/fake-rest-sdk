...
class FakeRestTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FakeRestTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FakeRestTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FakeRestTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = FakeRestTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FakeRestTestRunner.getenv(key)
        ...
        explain = FakeRestTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if FakeRestTestRunner._test_control is not None:
            return FakeRestTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FakeRestTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FakeRestTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FakeRestTestRunner.load_test_control()
...
def load_env_local():
    FakeRestTestRunner.load_env_local()


def env_override(m):
    return FakeRestTestRunner.env_override(m)


def entity_data(v):
    return FakeRestTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FakeRestTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FakeRestTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FakeRestTestRunner.load_test_control()


def live_client_options():
    return FakeRestTestRunner.live_client_options()


def live_delay_ms():
    return FakeRestTestRunner.live_delay_ms()
