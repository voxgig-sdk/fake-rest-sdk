...
//     (\`new FakeRestSDK(...)\` / \`FakeRestSDK.test(...)\` become
//     \`FakeRestSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"category":{"test01":{"id":"test01"}},"comment":{"test01":{"id":"test01"}},"post":{"test01":{"id":"test01"}},"product":{"test01":{"id":"test01"}},"todo":{"test01":{"id":"test01"}},"user":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FakeRestSDK(...)\` and any
// \`FakeRestSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FakeRestSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FakeRestSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
