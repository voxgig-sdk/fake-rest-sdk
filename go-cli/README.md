...
boru-driven command-line client **and** interactive REPL for the FakeRest
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fake-rest-cli)
make build
...
./fake-rest-cli list comment

# 5. Override the API base URL for a single call
FAKE_REST_BASE=https://api.example.com ./fake-rest-cli list category

# 6. No arguments -> interactive REPL
./fake-rest-cli
fake-rest> list category
fake-rest> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fake-rest-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/fake-rest-cli list category
   ```
...
Run with no arguments to open a REPL (prompt `fake-rest>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/fake-rest-cli`. |
...
(`./fake-rest-cli <expr>`) and interactively (the REPL), and expressions compose the
