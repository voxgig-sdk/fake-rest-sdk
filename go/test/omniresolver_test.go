...
import (
	...
	omni "github.com/voxgig-sdk/fake-rest-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FakeRestError); is {
				out := map[string]any{
					"name":    "FakeRestError",
...
func omniCtx(arg any, client *sdk.FakeRestSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FakeRestSDK); is {
			client = live
