...
module FakeRestUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = FakeRestResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FakeRestResponse.new(fetched)
    else
