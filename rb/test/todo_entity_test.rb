...
class TodoEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FAKE_REST_TEST_TODO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    todo_ref01_data = nil
    if todo_ref01_data_raw.length > 0
...
def todo_basic_setup(extra)
  ...
  entid_env_raw = ENV["FAKE_REST_TEST_TODO_ENTID"]
